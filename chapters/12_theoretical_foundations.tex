...
\footnotesize
\centering
\caption{State Transition Model Under RCO Validation Constraints}
\label{tab:state-transition}

\renewcommand{\arraystretch}{1.25}

\begin{tabularx}{\textwidth}{|p{2.5cm}|p{2.5cm}|X|p{2.5cm}|X|}
\hline
\textbf{Current State} & \textbf{Input} & \textbf{Condition} & \textbf{Next State} & \textbf{Validity Constraint} \\
\hline

$S_t$ & $D_t$ & Input received but not yet validated & No transition & Transition blocked until validation \\

\hline

$S_t$ & $D_t$ & RCO validation fails & $S_t$ (unchanged) & Invalid input must not affect state \\

\hline

$S_t$ & $D_t$ & RCO validation succeeds & $S_{t+1} = \mathcal{F}(S_t, D_t)$ & Only validated input drives transition \\

\hline

$S_t$ & Replayed $D_t$ & Lineage mismatch ($L_t \neq L_t'$) & $S_t$ (unchanged) & Replay attempts rejected \\

\hline

$S_t$ & $(D_t, S_t')$ & State mismatch ($S_t' \neq S_t$) & $S_t$ (unchanged) & State binding enforces context integrity \\

\hline

$S_t$ & $D_t$ & Insufficient validator signatures ($< t$) & $S_t$ (unchanged) & Threshold condition not satisfied \\

\hline

$S_t$ & $D_t$ & Aggregated signature invalid & $S_t$ (unchanged) & Cryptographic verification must succeed \\

\hline

$S_t$ & $\emptyset$ & No input & $S_t$ & System remains stable without input \\
