...
\begin{equation}
\mathcal{E}_1(D) = \texttt{\{k$_1$:v$_1$, k$_2$:v$_2$\}}, \quad
\mathcal{E}_2(D) = \texttt{\{k$_2$:v$_2$, k$_1$:v$_1$\}}
\end{equation}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/2-Dimentionals/validation_timing.png}
\caption{Comparison between traditional post-ingestion validation and RCO pre-ingestion validation, highlighting enforcement of integrity prior to persistence.}
\label{fig:validation-timing}
\end{figure}
...
\footnotesize
\centering
\caption{Comparison of RCO with Traditional Logging and Blockchain-Based Systems}
...
\hline
\textbf{Property} & \textbf{Traditional Logging Systems} & \textbf{Blockchain / Ledger Systems} & \textbf{RCO Protocol} \\
\hline

Validation Timing & Post-ingestion (after storage) & Post-commit (after block inclusion) & Pre-ingestion (before persistence) \\

\hline

Data Integrity Model & Best-effort or audit-based & Cryptographic chain integrity & Cryptographic validation + enforced admission control \\

\hline

State Awareness & No explicit state binding & Limited (transaction context only) & Explicit binding to system state $S_t$ \\

\hline

Replay Resistance & Weak (requires external checks) & Partial (depends on transaction uniqueness) & Strong via lineage $L_t$ and state binding $M_t$ \\

\hline

Trust Model & Centralized authority & Distributed consensus & Threshold-based distributed validation $(n, t, f)$ \\

\hline

Failure Handling & Detection after compromise & Forks / reorganization possible & Immediate rejection of invalid inputs \\

\hline

Data Admission Control & None (all data stored) & All valid transactions included & Only validated data admitted \\

\hline

Latency Characteristics & Low (no pre-validation) & High (consensus overhead) & Moderate (parallel validation) \\

\hline

Scalability & High but weak guarantees & Limited by consensus throughput & Scales with validator parallelism \\

\hline

Auditability & Requires external verification & Built-in but heavy & Built-in, lightweight, and verifiable \\

\hline

System Coupling & Decoupled from system state & Weak coupling & Strong feedback-coupled system model \\

\hline

Primary Use Case & Logging, monitoring & Financial transactions, ledgers & Real-time validated telemetry and control systems \\
